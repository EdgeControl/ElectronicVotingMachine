...
bool del=false,s1=false,s2=false,s3=false,verify = false,allow=false,tuff=false;
void page_1();
void page_2();
void page_3();
void page_4();
 ...
 while(Security){
  ...
  if(allow){
    ...
    delay(200);
      ...
      if (digitalRead(CAND9_PIN) == LOW) {
    digitalWrite(CAND9_LED,HIGH);
    digitalWrite(buzz,HIGH);
    incrementVote(81);
    showMessage("Vote Registered 2");
...
//        page_2();
  ...
  Serial.println("Enter psw");
    ...
    tuff=false;
     ...
     dell();
    checkButton(CAND1_PIN, 1, 0);
  checkButton(CAND2_PIN, 2, 1);
  checkButton(CAND3_PIN, 3, 2);
  checkButton(CAND4_PIN, 4, 3);
  checkButton(CAND5_PIN, 5, 4);
  checkButton(CAND6_PIN, 6, 5);
  checkButton(CAND7_PIN, 7, 6);
  checkButton(CAND8_PIN, 8, 7);
  checkButton(CAND9_PIN, 9, 8);
  checkButton(CAND10_PIN, 10, 9);
  checkButton(CAND11_PIN, 11, 10);
  checkButton(CAND12_PIN, 12, 11);
  checkButton(CAND13_PIN, 13, 12);
  checkButton(CAND14_PIN, 14, 13);
  checkButton(CAND15_PIN, 15, 14);
  if(tuff){
    Delete();
    tuff=false;
  }
...
void checkButton(int pin, int buttonNumber, int index) {
  bool currentState = digitalRead(pin);

  // Detect press (falling edge)
  if (lastState[index] == HIGH && currentState == LOW) {
...
void handleInput(int buttonNumber) {
  inputSequence[inputIndex++] = buttonNumber;
lcd.setCursor(z++,1);
lcd.print("*");
Serial.println("*");
